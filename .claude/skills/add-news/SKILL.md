...
4. If an image was given: resize it to 1600px wide JPEG with `sips` and write it to
   ...
   given, leave `image: null` and skip this step — do not invent or fetch one. Then run
   `npm run images:optimize` and commit the AVIF/WebP variants it writes alongside the
   JPEG (`npm run build` also regenerates them, so this is a courtesy, not a requirement).
